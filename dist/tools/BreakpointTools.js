...
export class BreakpointTools {
    ...
    registerTools(server) {
        ...
        server.registerTool("clear_breakpoint", {
        ...
        server.registerTool("toggle_breakpoint", {
    ...
    async handleSetBreakpoint(args) {
        try {
    ...
    async handleClearBreakpoint(args) {
        try {
            // Validate session exists
            const session = this.debugResources?.getSession(args.session_id);
            if (!session) {
            ...
            const breakpointIndex = session.breakpoints.findIndex((bp) => bp.id === args.breakpoint_id);
    ...
    async handleListBreakpoints(args) {
        try {
            // Validate session exists
            const session = this.debugResources?.getSession(args.session_id);
            if (!session) {
            ...
            const breakpointData = breakpoints.map((bp) => ({
                id: bp.id,
                file: bp.file,
                line: bp.line,
    ...
    async handleToggleBreakpoint(args) {
        try {
            // Validate session exists
            const session = this.debugResources?.getSession(args.session_id);
            if (!session) {
            ...
            // Find breakpoint
            const breakpoint = session.breakpoints.find((bp) => bp.id === args.breakpoint_id);
            if (!breakpoint) {
                throw new BreakpointNotFoundError(args.breakpoint_id);
            }
            // Update breakpoint
...
//# sourceMappingURL=BreakpointTools.js.map