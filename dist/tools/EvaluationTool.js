...
export class EvaluationTool {
    ...
    registerTools(server) {
        ...
        server.registerTool("watch_expression", {
        ...
        server.registerTool("list_watch_expressions", {
        ...
        server.registerTool("clear_watch_expression", {
    ...
    async handleWatchExpression(args) {
        try {
            // Validate session exists
            const session = this.debugResources?.getSession(args.session_id);
            if (!session) {
    ...
    async handleListWatchExpressions(args) {
        try {
            // Validate session exists
            const session = this.debugResources?.getSession(args.session_id);
            if (!session) {
    ...
    async handleClearWatchExpression(args) {
        try {
            // Validate session exists
            const session = this.debugResources?.getSession(args.session_id);
            if (!session) {
    ...
    async simulateNodeEvaluation(expression, _frameId) {
    ...
    async simulatePythonEvaluation(expression, _frameId) {
...
//# sourceMappingURL=EvaluationTool.js.map