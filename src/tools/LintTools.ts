...
import type { MCPDebuggerConfig, LintReport, MCPToolResult } from "../types/index.js";
...
export class LintTools {
  ...
  private async handleRunLint(args: {
    tool: string;
    paths?: string[];
    args?: string[];
    timeout_ms?: number;
  ...
  private async executeLintCommand(
    command: string,
    args: string[],
    timeout: number
  ...
  private parseLintOutput(stdout: string, stderr: string, _tool: string): Array<{
    file: string;
    line: number;
    column?: number;
    ...
    const issues: Array<{ file: string; line: number; column?: number; severity: 'error' | 'warning' | 'info'; message: string; rule?: string; code?: string; }> = [];
    const output = stdout + '\n' + stderr;
    ...
    for (const line of lines) {
      ...
      if (tsMatch) {
        issues.push({
          ...
          severity: tsMatch[4] as 'error' | 'warning' | 'info',
          message: tsMatch[6],
          code: `TS${tsMatch[5]}`
        });
        continue;
