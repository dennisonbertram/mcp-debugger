...
import type { MCPDebuggerConfig, DebugSession, MCPToolResult } from "../types/index.js";
// import removed: SessionAlreadyExistsError not used

export class DebugSessionTools {
  private config: MCPDebuggerConfig;
  private activeProcesses: Map<string, { process: any; session: DebugSession }> = new Map();
  ...
  private async handleOpenDebugSession(args: {
    kind: string;
    entry: string;
    args?: string[];
  ...
  private async handleCloseDebugSession(args: { session_id: string }): Promise<MCPToolResult> {
    try {
  ...
  private async handleListDebugSessions(args: { status?: string }): Promise<MCPToolResult> {
    try {
      const sessions = Array.from(this.activeProcesses.values())
        .map(({ session }) => session)
        .filter(session => {
          if (args.status === 'all' || !args.status) return true;
          if (args.status === 'active') return ['starting', 'running', 'paused'].includes(session.status);
  ...
  private async startDebugProcess(session: DebugSession): Promise<any> {
  ...
  async close(): Promise<void> {
    ...
    const closePromises = Array.from(this.activeProcesses.entries()).map(
      async ([sessionId, { process, session }]) => {
        try {
          if (process && !process.killed) {
            process.kill('SIGTERM');
          }
