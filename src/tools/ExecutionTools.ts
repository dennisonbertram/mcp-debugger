...
export class ExecutionTools {
  ...
  private async handleContinue(args: { session_id: string }): Promise<MCPToolResult> {
    try {
  ...
  private async handleStepInto(args: { session_id: string }): Promise<MCPToolResult> {
    try {
      // Validate session exists
      const session = this.debugResources?.getSession(args.session_id);
      if (!session) {
  ...
  private async handleStepOver(args: { session_id: string }): Promise<MCPToolResult> {
    try {
      // Validate session exists
      const session = this.debugResources?.getSession(args.session_id);
      if (!session) {
  ...
  private async handleStepOut(args: { session_id: string }): Promise<MCPToolResult> {
    try {
      // Validate session exists
      const session = this.debugResources?.getSession(args.session_id);
      if (!session) {
  ...
  private async handlePause(args: { session_id: string }): Promise<MCPToolResult> {
    try {
      // Validate session exists
      const session = this.debugResources?.getSession(args.session_id);
      if (!session) {
