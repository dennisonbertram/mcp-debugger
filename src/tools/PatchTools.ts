...
import type { MCPDebuggerConfig, FilePatch, MCPToolResult } from "../types/index.js";

export class PatchTools {
  private config: MCPDebuggerConfig;
  private pendingPatches: Map<string, FilePatch> = new Map();
  ...
  private async handleApplyPatch(args: {
    file: string;
    start?: number;
    end?: number;
    replacement?: string;
  ...
  private async applyPatch(patch: FilePatch): Promise<any> {
  ...
  async confirmPatch(patchId: string, confirmedBy: string): Promise<boolean> {
